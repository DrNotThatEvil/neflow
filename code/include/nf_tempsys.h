...
/// TODO: (2023-12-16, Tweak the NORMAL_ALLOWED_CHANGE rate, it's a bit to
/// sensitive) Allow the temp system access to the nf_state, so it can read the
/// PID settings.
// After that you can try running it with a pid for a bit! YAY
// Almost ready to get it like bolted up!

// ALSO IMPORTANT, make sure you expand the error state to turn everything
// important off! Currently it's only the 'normal' state doing some sanity
// checking (temprature rise is weird when heaters are never supposed to be on)
// but in other states the heater will get turned on so it's VERY important to
// turn it off if ERROR.
