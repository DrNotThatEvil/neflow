                      ...
                      _nf_memory_state_t *_memory)
{
    ...
    _profile_state->_profiles =
        (_nf_profile_t *)malloc(NUMBER_OF_PROFILES * sizeof(_nf_profile_t));
    ...
    for (uint i = 0; i < NUMBER_OF_PROFILES; i++)
    {
        for (uint j = 0; j < PROFILE_TARGETS_SIZE; j++)
        {
            ...
            _profile_state->_profiles[i].targets[j][0] =
                _memory->current_buffer->profile_data[i][j][0];
            _profile_state->_profiles[i].targets[j][1] =
                _memory->current_buffer->profile_data[i][j][1];
                     ...
                     _nf_profile_t *_profile)
{
    ...
    for (uint j = 0; j < PROFILE_TARGETS_SIZE; j++)
    {
        _profile_state->_memory->current_buffer->profile_data[offset][j][0] =
            _profile->targets[j][0];
        _profile_state->_memory->current_buffer->profile_data[offset][j][1] =
            _profile->targets[j][1];
    }
