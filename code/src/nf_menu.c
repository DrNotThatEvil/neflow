                  ...
                  _nf_memory_state_t *_memory, _nf_tempsys_t *_tempsys)
{
    ...
    _menu_state->btn_update_timeout =
        make_timeout_time_ms(NF_MENU_BTN_UPDATE_TIMEOUT_MS);
    _menu_state->scr_update_timeout =
        make_timeout_time_ms(_menu_state->refresh_ms);
...
void menu_gpio_callback(_nf_menu_t *_menu, uint gpio, uint32_t events)
{
    if (events & GPIO_IRQ_EDGE_FALL)
    {
        ...
        if (gpio == NF_MENU_BACK_BTN &&
            !_menu->_buttons[NF_BACK_BTN_INDEX].pressed)
        {
            _menu->_buttons[NF_BACK_BTN_INDEX].pressed = true;
            nf_menu_btn_handler(_menu, 0, false);

            return;
            //_state->_menu->menu_options[_state->_menu->current_menu_option].back_fn((void*)
            //_state->_menu, false);
        }

        if (gpio == NF_MENU_NEXT_BTN &&
            !_menu->_buttons[NF_NEXT_BTN_INDEX].pressed)
        {
            _menu->_buttons[NF_NEXT_BTN_INDEX].pressed = true;
            nf_menu_btn_handler(_menu, 1, false);

            return;
            //_state->_menu->menu_options[_state->_menu->current_menu_option].next_fn((void*)
            //_state->_menu, false);
        }

        if (gpio == NF_MENU_SELECT_BTN &&
            !_menu->_buttons[NF_SELECT_BTN_INDEX].pressed)
        {
            _menu->_buttons[NF_SELECT_BTN_INDEX].pressed = true;
            nf_menu_btn_handler(_menu, 2, false);

            return;
            //_state->_menu->menu_options[_state->_menu->current_menu_option].select_fn((void*)
            //_state->_menu, false);
        }
    }
...
{
    if (get_absolute_time() < _menu_state->scr_update_timeout)
    {
        return;
    }
    ...
    if (_menu_state->_state == MENU_STATE_ERROR)
    {
        ...
        if (_menu_state->_error_state.error_animation == 0)
        {
            ...
            ssd1306_draw_string(_menu_state->_disp_ptr, 5, 35, 1,
                                "Reset oven!");
        }
        ssd1306_show(_menu_state->_disp_ptr);
        _menu_state->scr_update_timeout = make_timeout_time_ms(1000);
        _menu_state->_error_state.error_animation =
            (_menu_state->_error_state.error_animation + 1) % 2;
    }
...
void menu_update_buttons(_nf_menu_t *_menu_state)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (_menu_state->_buttons[i].pressed &&
            !_menu_state->_buttons[i].held &&
            !_menu_state->_buttons[i].released)
        {
            _menu_state->_buttons[i].held = true;
        }

        if (_menu_state->_buttons[i].pressed && _menu_state->_buttons[i].held &&
            !_menu_state->_buttons[i].released)
        {
            // Button being held repeat event here.
            // only back and next should have repeat events
            if (_menu_state->_buttons[i].held_count >
                NF_MENU_BTN_REPEAT_UNTIL_HELD)
            {
            ...
            {
                _menu_state->_buttons[i].held_count =
                    (_menu_state->_buttons[i].held_count + 1);
            }
            continue;
        }

        if (_menu_state->_buttons[i].released &&
            _menu_state->_buttons[i].pressed)
        {
            // Fire released event here.. if i ever had one... (yes i'm
            // picuteing the farily odd parents meme atm...)
            _menu_state->_buttons[i].released = false;
            _menu_state->_buttons[i].held = false;
            _menu_state->_buttons[i].held_count = 0;
            _menu_state->_buttons[i].pressed = false;
...
void nf_menu_render(_nf_menu_t *_menu)
{
    ...
    (*_menu->current_screen)
        ->fnptrs.on_render(_menu, _menu->_disp_ptr,
                           (*_menu->current_screen)->extra_data);
}
