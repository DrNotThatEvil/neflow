...
void nf_main_menu_render(_nf_menu_t *menu_state, ssd1306_t *disp_ptr,
                         void *extra_data)
{
    _nf_main_menu_state_t *main_menu_state =
        ((_nf_main_menu_state_t *)extra_data);

    char temp_str[20];
    sprintf(temp_str, "Temp: %.2f", menu_state->cur_temp);
    ssd1306_draw_string(disp_ptr, 5, 5, 1, temp_str);

    ssd1306_draw_line(disp_ptr, 1 + (5 * main_menu_state->line_offset), 1,
                      5 + (5 * main_menu_state->line_offset), 1);
    main_menu_state->line_m = (main_menu_state->line_m + 1) % 5;
    if (main_menu_state->line_m == 0)
    {
        main_menu_state->line_offset = (main_menu_state->line_offset + 1) % 25;
    }
    ...
    char str[20];
    if (main_menu_state->selected == PROFILES_INDEX)
    {
        sprintf(str, "%s", "Profiles\0");
    }
    else if (main_menu_state->selected == CALIBRATION_INDEX)
    {
        sprintf(str, "%s", "Calibration\0");
    }
    else if (main_menu_state->selected == CONFIG_INDEX)
    {
        sprintf(str, "%s", "Config\0");
#if ENABLE_TEST_MENU == 1
    }
    else if (main_menu_state->selected == TEST_MENU_INDEX)
    {
...
void nf_main_menu_btn_handler(_nf_menu_t *menu_state, uint btn, bool repeat,
                              void *extra_data)
{
    uint *data = &(((_nf_main_menu_state_t *)extra_data)->selected);
...
void nf_main_menu_init(_nf_menu_t *_menu_state)
{
    ...
    _nf_menu_screen_t *main_screen =
        (_nf_menu_screen_t *)malloc(sizeof(_nf_menu_screen_t));
    main_screen->id = MAIN_MENU_SCREEN_ID;
    main_screen->autoclear = true;
    main_screen->extra_data = malloc(sizeof(_nf_main_menu_state_t));

    _nf_main_menu_state_t *main_menu_state =
        (_nf_main_menu_state_t *)main_screen->extra_data;
    main_menu_state->selected = PROFILES_INDEX;
    main_menu_state->line_offset = 0;
    main_menu_state->line_m = 0;
