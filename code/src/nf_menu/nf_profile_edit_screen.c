...
void nf_profile_edit_render(_nf_menu_t *menu_state, ssd1306_t *disp_ptr,
                            void *extra_data)
{
    _nf_profile_edit_screen_state_t *profile_edit_state =
        ((_nf_profile_edit_screen_state_t *)extra_data);
    ...
    draw_edit_item(
        disp_ptr, 0,
        (profile_edit_state->editing == 1 && !profile_edit_state->adjust),
        "Back");
    draw_edit_item(
        disp_ptr, 11,
        (profile_edit_state->editing == 2 && !profile_edit_state->adjust),
        "Target temp:");

    char str[20];
    sprintf(str, "%d",
            (profile_edit_state->current_profile->targets[(selected - 1)][0]));
    draw_edit_value(
        disp_ptr, 22,
        (profile_edit_state->editing == 2 && profile_edit_state->adjust), str);

    draw_edit_item(
        disp_ptr, 34,
        (profile_edit_state->editing == 3 && !profile_edit_state->adjust),
        "Target time:");
    sprintf(str, "%d",
            (profile_edit_state->current_profile->targets[(selected - 1)][1]));
    draw_edit_value(
        disp_ptr, 45,
        (profile_edit_state->editing == 3 && profile_edit_state->adjust), str);
}
                                 ...
                                 void *extra_data)
{
    ...
    if (btn == 0)
    {
        if (profile_edit_state->editing == 0)
        {
            ...
            {
                profile_edit_state->selected_value =
                    (profile_edit_state->selected_value - 1) % 6;
            }
            return;
        }
        ...
        if (!profile_edit_state->adjust)
        {
            ...
            {
                profile_edit_state->editing =
                    (profile_edit_state->editing - 1) % 3;
            }
        }
        ...
        if (profile_edit_state->adjust)
        {
            ...
            {
                uint *value_prev = &(profile_edit_state->current_profile
                                         ->targets[selected - 1][1]);
    ...
    if (btn == 2 && !repeat)
    {
        ...
        if (profile_edit_state->selected_value == 5 &&
            profile_edit_state->editing == 0)
        {
            uint prev_time = 0;
            ...
            {
                uint *temp =
                    &(profile_edit_state->current_profile->targets[i][0]);
                uint *time =
                    &(profile_edit_state->current_profile->targets[i][1]);
            ...
            if (!has_failed)
            {
                nf_profile_save(profile_edit_state->_profile_state,
                                profile_edit_state->current_profile);
        ...
        {
            if (profile_edit_state->selected_value > 0 &&
                profile_edit_state->selected_value < 5)
            {
                ...
                if (selected > 0)
                {
                    uint *value_prev = &(profile_edit_state->current_profile
                                             ->targets[selected - 1][1]);
                    uint *value = &(profile_edit_state->current_profile
                                        ->targets[selected][1]);
                               ...
                               _nf_profile_state_t *_profile_state)
{
    ...
    _nf_menu_screen_t *profile_edit_screen =
    ...
    profile_edit_screen->extra_data =
        malloc(sizeof(_nf_profile_edit_screen_state_t));

    _nf_profile_edit_screen_state_t *profile_edit_state =
        (_nf_profile_edit_screen_state_t *)profile_edit_screen->extra_data;
