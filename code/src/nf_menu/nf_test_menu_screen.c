...
{
    _nf_test_menu_state_t *test_state =
        ((_nf_test_menu_state_t *)t_ptr->user_data);
    ...
    if (test_state->countdown_time == 0 || t_cancel)
    {
        ...
        {
            note_t CONFIRM_DONE[] = {
                {NOTE_A7, 64},   {NOTE_A6, 64}, {NOTE_A7, 64}, {NOTE_A6, 64},
                {NOTE_A7, 64},   {NOTE_A6, 64}, {NOTE_A7, 64}, {NOTE_A6, 64},
                {NOTE_A7, 64},   {NOTE_A6, 64}, {NOTE_A7, 64}, {NOTE_A6, 64},
                {NOTE_A7, 64},   {NOTE_A6, 64}, {NOTE_A7, 64}, {NOTE_A6, 64},
                {MELODY_END, 0},
        ...
        {
            note_t REJECT[] = {
                {NOTE_CM1, 128}, {REST, 128},     {NOTE_CM1, 128}, {REST, 128},
                {NOTE_CM1, 128}, {REST, 128},     {NOTE_CM1, 128}, {REST, 128},
                {REST, 8},       {MELODY_END, 0},
            };
        ...
        if (test_state->selected_option < 4)
        {
            const uint INDEX_TO_PIN[] = {NF_SSR0_PIN, NF_SSR1_PIN, NF_FAN0_PIN,
                                         NF_FAN1_PIN};
        ...
        else
        {
            if (!test_state->cancel)
            {
                ...
                test_state->animation_timeout =
                    make_timeout_time_ms(ANIMATION_TIMEOUT);
            }
        }
...
void nf_test_render(_nf_menu_t *menu_state, ssd1306_t *disp_ptr,
                    void *extra_data)
{
    char str[20];
    char str2[20];
    ...
    const char *cleared_strings[4] = {"Memory Cleared", "Please", "Reset",
                                      "Thanks!!"};
    if (test_state->cleaned_memory > 0)
    {
        sprintf(str2, "%s", cleared_strings[test_state->cleared_animation]);
        ssd1306_draw_string(disp_ptr, 5,
                            12 + (test_state->cleared_animation * 10), 1, str2);
    ...
    sprintf(str, "%s: %s", (test_state->testing ? "Testing: " : "Test: "),
            options[test_state->selected_option]);
    ssd1306_draw_string(disp_ptr, 5, 35 - (test_state->testing ? 0 : 10), 1,
                        str);

    draw_next_arrow(disp_ptr);
}

void nf_test_btn_handler(_nf_menu_t *menu_state, uint btn, bool repeat,
                         void *extra_data)
{
    _nf_test_menu_state_t *test_state = ((_nf_test_menu_state_t *)extra_data);
    if (test_state->cleaned_memory > 0)
    ...
    if (!test_state->testing)
    {
        ...
        if (btn == 1)
        {
            test_state->selected_option =
                ((test_state->selected_option) + 1) % 5;
            return;
        }
        ...
        if (btn == 2)
        {
            ...
            if (test_state->selected_option < 4)
            {
                ...
                const uint INDEX_TO_PIN[] = {NF_SSR0_PIN, NF_SSR1_PIN,
                                             NF_FAN0_PIN, NF_FAN1_PIN};
                gpio_put(INDEX_TO_PIN[test_state->selected_option], 1);
            }

            add_repeating_timer_ms(-900, testing_countdown, test_state,
                                   test_state->timer);
            return;
        }
    }
    ...
    if (test_state->testing)
    {
        ...
        {
            test_state->selected_option =
                ((test_state->selected_option) + 1) % 3;
            return;
        }
...
void nf_test_menu_init(_nf_menu_t *_menu_state, _nf_memory_state_t *memory)
{
    ...
    _nf_test_menu_state_t *test_menu_state =
        (_nf_test_menu_state_t *)test_screen->extra_data;
    test_menu_state->_memory = memory;
    test_menu_state->timer =
        (struct repeating_timer *)malloc(sizeof(struct repeating_timer));
    test_menu_state->selected_option = 0;
    test_menu_state->cleaned_memory = 0;
    test_menu_state->cleared_animation = 0;
    test_menu_state->animation_timeout =
        make_timeout_time_ms(ANIMATION_TIMEOUT);
    ...
    _nf_menu_screen_fn_ptrs_t test_screen_fns = {.on_render = nf_test_render,
