...
void nf_profile_run_render(_nf_menu_t *_menu_state, ssd1306_t *disp_ptr,
                           void *extra_data)
{
    _nf_profile_run_screen_state_t *profile_run_state =
        ((_nf_profile_run_screen_state_t *)extra_data);
    if (!profile_run_state->_running)
    {
        ...
        ssd1306_draw_string(_menu_state->_disp_ptr, 2, 15, 1,
                            "Please remain near!");
        ssd1306_draw_string(_menu_state->_disp_ptr, 2, 35, 1,
                            "Open door when asked.");
        ...
        profile_run_state->_finished_door_state < 5)
    {
        ...
        {
            ssd1306_draw_string(_menu_state->_disp_ptr, 5, 10, 1,
                                "!! Finished !!");
            ssd1306_draw_string(_menu_state->_disp_ptr, 5, 35, 1, "Open door!");
            ssd1306_draw_string(_menu_state->_disp_ptr, 5, 50, 1,
                                "(Press enter after)");
        }

        if (get_absolute_time() > profile_run_state->_beep_timeout)
        {
            tone(_menu_state->_tonegen,
                 notes[profile_run_state->_finished_door_state], 150);

            profile_run_state->_beep_timeout =
                make_timeout_time_ms(NF_PROFILE_RUN_BEEP_TIMEOUT);
            profile_run_state->_finished_door_state =
                (profile_run_state->_finished_door_state + 1) % 5;
        }
    ...
    {
        double l =
            ((((double)profile_run_state->current_profile->targets[i][1]) /
              _graph->_sec_high_value) *
             (NF_GRAPH_MAX_X - NF_GRAPH_ZERO_X));

        for (uint8_t y = NF_GRAPH_ZERO_Y; y > NF_GRAPH_MAX_Y; y -= 10)
        {
            ssd1306_draw_line(disp_ptr, l + NF_GRAPH_ZERO_X, y,
                              l + NF_GRAPH_ZERO_X, y - 5);
        }
    }
}

void nf_profile_run_on_active_handler(_nf_menu_t *_menu_state, void *extra_data,
...
void nf_profile_run_btn_handler(_nf_menu_t *_menu_state, uint btn, bool repeat,
                                void *extra_data)
{
    _nf_profile_run_screen_state_t *profile_run_state =
        ((_nf_profile_run_screen_state_t *)extra_data);
                                ...
                                _nf_profile_state_t *_profile_state)
{
    ...
    profile_run_screen->extra_data =
        malloc(sizeof(_nf_profile_run_screen_state_t));

    _nf_profile_run_screen_state_t *profile_run_state =
        (_nf_profile_run_screen_state_t *)profile_run_screen->extra_data;
    ...
    _nf_graph_state_t *_graph =
        (_nf_graph_state_t *)malloc(sizeof(_nf_graph_state_t));
    _graph->_sample_count = 0;
    _graph->_sum = 0.0;
    _graph->_avg_index = 0;
