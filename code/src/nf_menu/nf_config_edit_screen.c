...
void nf_config_edit_render(_nf_menu_t *menu_state, ssd1306_t *disp_ptr,
                           void *extra_data)
{
    _nf_config_edit_screen_state_t *config_edit_state =
        ((_nf_config_edit_screen_state_t *)extra_data);
    ...
    draw_edit_item(
        disp_ptr, 0,
        (config_edit_state->editing == 1 && !config_edit_state->adjust),
        "Back");
    draw_edit_item(
        disp_ptr, 11,
        (config_edit_state->editing == 2 && !config_edit_state->adjust),
        config_options[selected]);

    float value = config_edit_state->_memory_state->current_buffer
                      ->pid_data[set - 1][selected - 1];
    char str[20];
    sprintf(str, "%.2f", value);
    draw_edit_value(
        disp_ptr, 22,
        (config_edit_state->editing == 2 && config_edit_state->adjust), str);

    draw_edit_item(
        disp_ptr, 34,
        (config_edit_state->editing == 3 && !config_edit_state->adjust),
        "Change:");
    sprintf(str, "%.2f", change_values[change]);
    draw_edit_value(
        disp_ptr, 45,
        (config_edit_state->editing == 3 && config_edit_state->adjust), str);
}
                                ...
                                void *extra_data)
{
    ...
    if (btn == 0)
    {
        if (config_edit_state->editing == 0)
        {
            ...
            {
                config_edit_state->selected_value =
                    (config_edit_state->selected_value - 1) % 4;
            }
            return;
        }
        ...
        if (!config_edit_state->adjust)
        {
            ...
            {
                config_edit_state->editing =
                    (config_edit_state->editing - 1) % 3;
            }
        }
        ...
        if (config_edit_state->adjust)
        {
            if (config_edit_state->editing == 2)
            {
                ...
                float *value = &(config_edit_state->_memory_state
                                     ->current_buffer->pid_data[set][selected]);

                if (*value > 0.01f)
                {
                    if ((*value - change_values[config_edit_state->change]) >
                            0.01f &&
                        (*value - change_values[config_edit_state->change]) <
                            100.0f)
                    {
                        *value =
                            (*value - change_values[config_edit_state->change]);
            ...
            if (config_edit_state->editing == 3)
            {
                ...
                {
                    config_edit_state->change =
                        (config_edit_state->change - 1) % 3;
                }
            }
        }
        /*
        if(config_edit_state->adjust) {
            ...
            uint* value =
        &(profile_edit_state->current_profile->targets[selected][edit]);
            ...
            if(edit == 0 || selected == 0) {
                ...
                uint* value_prev =
        &(profile_edit_state->current_profile->targets[selected-1][1]);
    ...
    if (btn == 2 && !repeat)
    {
        ...
        if (config_edit_state->editing == 0)
        {
            ...
            {
                if (config_edit_state->selected_value > 0 &&
                    config_edit_state->selected_value < 3)
                {
                    config_edit_state->selected_set =
                        config_edit_state->selected_value;
                    config_edit_state->selected_value = 0;
                ...
                if (config_edit_state->selected_value == 3)
                {
                    ...
                    nf_memory_save_with_callback(
                        config_edit_state->_memory_state, nf_config_edit_cb,
                        (void *)menu_state);
                }
            }
                              ...
                              _nf_memory_state_t *_memory_state_ptr)
{
    ...
    _nf_menu_screen_t *config_edit_screen =
        (_nf_menu_screen_t *)malloc(sizeof(_nf_menu_screen_t));

    config_edit_screen->id = CONFIG_SCREEN_ID;
    config_edit_screen->autoclear = true;
    config_edit_screen->extra_data =
        malloc(sizeof(_nf_config_edit_screen_state_t));

    _nf_config_edit_screen_state_t *config_edit_state =
        (_nf_config_edit_screen_state_t *)config_edit_screen->extra_data;
